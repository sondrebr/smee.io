...
const [
  ...
  const gl_digest = (req) => {
    // Sign GitLab webhooks with secret tokens
    const timestamp = req.headers['X-Gitlab-Timestamp']
    const webhookUuid = req.headers['x-gitlab-webhook-uuid']
    const body = req.rawBody
    const key = req.headers["x-gitlab-token"]

    const hmac = createHmac('sha256', key)
    hmac.update(timestamp.concat('.', webhookUuid, '.', body))
  ...
  fastify.post('/:channel', {
    ...
    schema: {
      ...
      headers: {
        ...
        properties: {
          ...
          'x-gitlab-token': { type: 'string', description: 'webhook secret' }
        },
    ...
    // Sign GitLab webhooks with secret tokens
    if (req.headers['x-gitlab-token'] !== undefined) {
      req.headers['X-Gitlab-Timestamp'] = Date.now().toString()
      req.headers['X-Gitlab-Signature-256'] = 'sha256='.concat(gl_digest(req))
      delete req.headers['x-gitlab-token']
    }
    await bus.emit(
      req.params.channel,
      {
